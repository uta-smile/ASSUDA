...
from data import CreateTrgDataLoader, CreateSrcDataLoader
...
from model import CreateModel, CreateDiscriminator
...
def main():
    ...
    model.eval()
    model.cuda(args.gpu_model)
    model_D = CreateDiscriminator(args)
    model_D.eval()
    model_D.cuda(args.gpu_model)
    
    targetloader = CreateTrgDataLoader(args)
    
    for index, batch in enumerate(zip(targetloader)):
        
        if index % 100 == 0:
            print('%d processd' % index)
        image, _, name = batch[0]
        
        image = Variable(image).cuda(args.gpu_model)
        output = model(image)
